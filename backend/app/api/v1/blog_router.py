import sqlite3
from fastapi import APIRouter, Depends, HTTPException, status
from typing import List

from app.db.database import get_db
from app.schemas.blog import BlogCreate, BlogUpdate, BlogResponse
from app.repositories.blog_repository import BlogRepository
from app.core.logging import logger
from app.core.auth import Principal
from app.core.dependencies import authorize, authorize_project, get_current_principal

router = APIRouter(prefix="/blogs", tags=["Blog Management"])

def get_blog_repo(conn: sqlite3.Connection = Depends(get_db)) -> BlogRepository:
    return BlogRepository(conn)

@router.post("", response_model=BlogResponse, status_code=status.HTTP_201_CREATED)
def create_blog(
    blog: BlogCreate,
    repo: BlogRepository = Depends(get_blog_repo),
    principal: Principal = Depends(get_current_principal),
):
    authorize(principal, "reviewer" if blog.status == "published" else "writer")
    authorize_project(principal, blog.project_id)
    logger.info("Creating new blog | title={}", blog.title)
    try:
        blog_id = repo.create(blog)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return repo.get_by_id(blog_id)

@router.get("", response_model=List[BlogResponse])
def list_blogs(
    project_id: str | None = None,
    repo: BlogRepository = Depends(get_blog_repo),
    principal: Principal = Depends(get_current_principal),
):
    authorize(principal, "writer")
    if project_id:
        authorize_project(principal, project_id)
    elif "*" not in principal.projects:
        raise HTTPException(status_code=403, detail="Project scope is required")
    return repo.list_all(project_id=project_id)

@router.get("/{blog_id}", response_model=BlogResponse)
def get_blog(
    blog_id: int,
    repo: BlogRepository = Depends(get_blog_repo),
    principal: Principal = Depends(get_current_principal),
):
    authorize(principal, "writer")
    blog = repo.get_by_id(blog_id)
    if not blog:
        raise HTTPException(status_code=404, detail="Blog not found")
    authorize_project(principal, blog["project_id"])
    return blog

@router.put("/{blog_id}", response_model=BlogResponse)
def update_blog(
    blog_id: int,
    blog: BlogUpdate,
    repo: BlogRepository = Depends(get_blog_repo),
    principal: Principal = Depends(get_current_principal),
):
    authorize(principal, "writer")
    current = repo.get_by_id(blog_id)
    if current is None:
        raise HTTPException(status_code=404, detail="Blog not found")
    authorize_project(principal, current["project_id"])
    logger.info("Updating blog | id={}", blog_id)
    try:
        success = repo.update(blog_id, blog)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if not success:
        raise HTTPException(status_code=404, detail="Blog not found or no changes made")
    return repo.get_by_id(blog_id)

@router.delete("/{blog_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_blog(
    blog_id: int,
    repo: BlogRepository = Depends(get_blog_repo),
    principal: Principal = Depends(get_current_principal),
):
    authorize(principal, "writer")
    current = repo.get_by_id(blog_id)
    if current is None:
        raise HTTPException(status_code=404, detail="Blog not found")
    authorize_project(principal, current["project_id"])
    logger.info("Deleting blog | id={}", blog_id)
    try:
        success = repo.delete(blog_id)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if not success:
        raise HTTPException(status_code=404, detail="Blog not found")
